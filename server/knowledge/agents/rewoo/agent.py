...
from server.knowledge.agents.base import BaseAgent
from server.knowledge.agents.io import AgentOutput, AgentType, BaseScratchPad
from server.knowledge.agents.tools import BaseTool
from server.knowledge.base import Document, Node, Param
from server.knowledge.indices.qa.citation import CitationPipeline
from server.knowledge.indices.splitters import TokenSplitter
from server.knowledge.llms import BaseLLM, PromptTemplate
    ...
    def _run_plugin(
        ...
        if "[" not in tool_call:
            ...
            try:
                ...
                if isinstance(tool_response, Document):
                    result["evidence"] = tool_response.text
                else:
                    result["evidence"] = str(tool_response)
            except ValueError:
