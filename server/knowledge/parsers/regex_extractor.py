...
from server.knowledge.base import BaseComponent, Document, ExtractorOutput, Param
...
class RegexExtractor(BaseComponent):
    ...
    def __init__(self, pattern: str | list[str], **kwargs):
        ...
        self.compiled = [re.compile(p) for p in self.pattern]

    @staticmethod
    def map_output(text, output_map: dict[str, str] | Callable[[str], str]) -> str:
        """
    ...
    def run_raw(self, text: str) -> ExtractorOutput:
        """
        Matches the raw text against the pattern and runs the output mapping, returning
        ...
        """
        output: list[str] = sum([c.findall(text) for c in self.compiled], [])
        output = [self.map_output(text, self.output_map) for text in output]

        return ExtractorOutput(
            text=output[0] if output else "", matches=output, metadata={"origin": "RegexExtractor"},
        )
