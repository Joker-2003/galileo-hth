...
from server.knowledge.base import BaseComponent, Document
...
class Thought(BaseComponent):
    ...
    ```python
    >> from server.knowledge.pipelines.cot import Thought
    >> thought = Thought(
...
class ManualSequentialChainOfThought(BaseComponent):
    ...
    **Create and run a chain of thought without "+" operator:**
    ...
    ```pycon
    >>> from server.knowledge.pipelines.cot import Thought, ManualSequentialChainOfThought
    >>> llm = LCAzureChatOpenAI(...)
    >>> thought1 = Thought(
    >>>    prompt="Word {word} in {language} is ",
    >>>    post_process=lambda string: {"translated": string},
